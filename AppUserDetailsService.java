...
public class AppUserDetailsService implements UserDetailsService {
	...
	public boolean signup(User user) throws UserAlreadyExistsException {
		...
		if (newUser.isPresent()) {
			...
			User newuser = new User(0, user.getFirstname(),user.getLastname(),user.getPassword(),user.getAge(),user.getWeight(),user.getEmail(),user.getContactnumber(),user.getBloodgroup(),user.getGender(),user.getAddressList(),roles);
